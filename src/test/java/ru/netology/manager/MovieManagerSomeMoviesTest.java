...
class MovieManagerSomeMoviesTest {
   ...
   private MovieInfo twelfth = new MovieInfo(12, "Форма воды", "Драма", "url12");
    ...
    void addAndGet() {
        ...
        MovieInfo[] returned = new MovieInfo[]{third, fourth, fifth, sixth, seventh, eighth, ninth, tenth, eleventh, twelfth, thirteenth};
    ...
    void getLast() {
        ...
        MovieManager manager = new MovieManager(repository,4);

        MovieInfo[] returned = new MovieInfo[]{eighth, ninth, tenth, eleventh, twelfth};
        doReturn(returned).when(repository).findAll();

        MovieInfo[] actual = manager.getLastAdded();
        MovieInfo[] expected = new MovieInfo[]{twelfth, eleventh, tenth, ninth};
